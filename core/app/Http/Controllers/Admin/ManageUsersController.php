...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalReferralUser = User::where('ref_by', $id)->count();
        $totalReferralCommission = Commissions::where('user_id',$user->id)->where('mark',1)->sum('amount');
        $totalLevelCommission = Commissions::where('user_id',$user->id)->where('mark',2)->sum('amount');
        $totalPinGenerate = Pin::where('generate_user_id',$user->id)->count();
        $totalUsedPin = Pin::where('user_id',$user->id)->where('status', 1)->count();

        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','countries', 'refer_by', 'totalReferralUser', 'totalReferralCommission', 'totalLevelCommission', 'totalPinGenerate', 'totalUsedPin'));
    }
    ...
    public function referralCommission(Request $request, $id)
    {
        ...
        if ($request->search) {
            ...
            $commissions = Commissions::where('user_id', $user->id)->where('trx', $search)->where('mark', 1)->with('user')->orderBy('id','desc')->paginate(getPaginate());
        ...
        $commissions = Commissions::with('user')->where('user_id', $user->id)->where('mark', 1)->orderBy('id','desc')->paginate(getPaginate());
    ...
    public function levelCommission(Request $request, $id)
    {
        ...
        if ($request->search) {
            ...
            $commissions = Commissions::where('user_id', $user->id)->where('trx', $search)->where('mark', 2)->with('user')->orderBy('id','desc')->paginate(getPaginate());
            $emptyMessage = 'No Commissions';
            return view('admin.reports.commissions', compact('pageTitle', 'search', 'commissions', 'emptyMessage'));
        }
        $pageTitle = "Level Commissions Log - ".$user->username;
        $emptyMessage = "No commissions found";
        $commissions = Commissions::with('user')->where('user_id', $user->id)->where('mark', 2)->orderBy('id','desc')->paginate(getPaginate());
        return view('admin.reports.commissions', compact('pageTitle', 'commissions', 'emptyMessage'));
    }
    ...
    public function referralLog($id)
    {
        ...
        $order = PlanSubscription::where('user_id', $userData->id)->first();
        $pageTitle = 'Referral log : ' . $userData->username;
        $emptyMessage = 'No referral user found';
        $users  = User::where('ref_by', $id)->latest()->paginate(getPaginate());
        return view('admin.users.referral', compact('pageTitle', 'emptyMessage', 'users', 'order', 'userData'));
    }

     public function userLevelReferrals(Request $request, $userId)
    {
        $levelId = $request->level;
        $userDataInfo = User::findOrFail($userId);
        $pageTitle = $userDataInfo->username ." Level ". $levelId . " Referred  User";
        $emptyMessage =  "No data found";
        $order = PlanSubscription::where('user_id', $userDataInfo->id)->first();
        ...
        $myReferral = $this->showBelow($userDataInfo->id);
        ...
        return view('admin.users.referral_level', compact('pageTitle', 'emptyMessage', 'nextArray', 'levelId', 'order', 'userDataInfo'));
    }
    ...
    public function updatePaymentStatus(Request $request){
        ...
        $trx = getTrx();
        //get payment date
        $paymentData = ReturnPayment::where('id',$payment_id)->first();
        $paymentData->amount = $amount;
        $paymentData->status = $status;
        $paymentData->date_of_payment = date("Y-m-d H:i:s");

        $transaction = new Transaction();
        $transaction->user_id = $paymentData->user_id;
        $transaction->amount = $amount;
        $transaction->trx_type = '+';
        $transaction->trx =  $trx;
        $transaction->details = 'Return Payment For '. $paymentData->date;
        $transaction->save();
